...
def _plot_h2_massfraction_particles(sg, plot_annotate = None, savefig_txt_in = None,      # SWIFTGalaxy object containing all the integrated properties + access the particles from the snapshot file
    ...
    h2_mass_rich_50     = np.sum(sg.gas.mass_gas_h2[trim_mask][h2_mass_fractions[trim_mask] > cosmo_quantity(h2_rich_fraction, u.dimensionless, comoving=False, scale_factor=sg.metadata.a, scale_exponent=0)])
    ...
    mask_sort = np.argsort(h2_mass_fractions[h2_mass_fractions > 0])
    
    
    
    #---------------------------
    # Graph initialising and base formatting
    fig, (ax1, ax2, ax3) = plt.subplots(nrows=3, ncols=1, figsize=[10/3, 6.5], sharex=True, sharey=False)
    plt.subplots_adjust(wspace=0.4, hspace=0.4)
    
    #--------------
    # histogram in log
    bin_step = 0.1
    hist_bins = np.arange(-3, 1.1, bin_step)
    ...
    ax2.hist(np.log10(h2_mass_fractions[h2_mass_fractions>0]), bins=hist_bins, weights=h2_mass_fractions[h2_mass_fractions>0]/h2_mass_particle_50)
    
    # Plot contribution to H2 mass in 50 pkpc (cumulative version of above)
    ax3.hist(np.log10(h2_mass_fractions[h2_mass_fractions>0]), bins=hist_bins, weights=h2_mass_fractions[h2_mass_fractions>0]/h2_mass_particle_50, histtype="step", cumulative=True)
    ...
    # General formatting
    ax1.set_xlim(-3, 1)
    ax1.set_xlabel('log10 H2 mass fraction in gas particles 50 pkpc')
    ax1.set_yscale("log")
    ax2.set_yscale("log")
...
def _analyse_rich_particles_mass_change(sg, plot_annotate = None, savefig_txt_in = None,      # SWIFTGalaxy object containing all the integrated properties + access the particles from the snapshot file
                      ...
                      # fig settings
                        analysis_radius  = 50,           # pkpc, radius to mask
                        h2_rich_fraction = 0.5,          # mass fraction that is considered 'H2 rich' 
                      #=====================================
                      print_progress = False,
                        debug = False):
...
def _plot_rich_particles_mass_change(output_data = 0, 
                                    #=====================================
                                    showfig       = False,
                                    savefig       = True,
                                      file_format = 'pdf',
    ...
    raise Exception('current pause: see what output_data is')
    ...
    df = pd.DataFrame(data={'soap_index_list': soap_index_list, 'soap_H2mass50': soap_H2mass50, 'h2_mass_particle_50_list': h2_mass_particle_50_list, 'h2_mass_rich_50_list': h2_mass_rich_50_list})        
    
    print('\n-------------------------------')
    print('Median particle / SOAP difference:   %'%np.median(df['h2_mass_particle_50_list']/df['soap_H2mass50']))
    print('   Max particle / SOAP difference:   %'%np.max(df['h2_mass_particle_50_list']/df['soap_H2mass50']))
    print('   Min particle / SOAP difference:   %'%np.min(df['h2_mass_particle_50_list']/df['soap_H2mass50']))
    print(' ')
    print('Median H2-rich / particle difference:   %'%np.median(df['h2_mass_rich_50_list']/df['h2_mass_particle_50_list']))
    print('   Max H2-rich / particle difference:   %'%np.max(df['h2_mass_rich_50_list']/df['h2_mass_particle_50_list']))
    print('   Min H2-rich / particle difference:   %'%np.min(df['h2_mass_rich_50_list']/df['h2_mass_particle_50_list']))
    print(' ')
    ...
    ax1.scatter(df['h2_mass_particle_50_list'], df['soap_H2mass50'], zorder=20)
    
    # scatter2 in log log: particle H2 vs gas rich particle H2
    ax2.scatter(df['h2_mass_particle_50_list'], df['h2_mass_rich_50_list'], zorder=20)
    ...
    if savefig:
        ...
        plt.savefig("%s/etg_swiftgalaxy_analysis/h2_soap_particle_rich_particle/%s_h2masschange_soap_particle_rich_%i_%s.%s" %(fig_dir, save_folder_visual, output_data['run_name'], savefig_txt_save, file_format), format=file_format, bbox_inches='tight', dpi=600)    
        print("\n  SAVED: %s/etg_swiftgalaxy_analysis/h2_soap_particle_rich_particle/%s_h2masschange_soap_particle_richs_%i_%s.%s" %(fig_dir, save_folder_visual, output_data['run_name'], savefig_txt_save, file_format)) 
    if showfig:
        plt.show()
    plt.close()
...
sgs = SWIFTGalaxies(sample_input['virtual_snapshot_file'], SOAP(sample_input['soap_catalogue_file'], soap_index=soap_indicies_sample, ), auto_recentre=False,
                    preload={"gas.coordinates", "gas.masses", "gas.element_mass_fractions.hydrogen", "gas.species_fractions.H2", "stars.coordinates", "stars.masses", },)       
output_data = tqdm(sgs.map(_plot_h2_massfraction_particles))
...
"""sgs = SWIFTGalaxies(sample_input['virtual_snapshot_file'], SOAP(sample_input['soap_catalogue_file'], soap_index=soap_indicies_sample, ), auto_recentre=False,
                    preload={"gas.coordinates", "gas.masses", "gas.element_mass_fractions.hydrogen", "gas.species_fractions.H2", "stars.coordinates", "stars.masses", },)       
output_data = sgs.map(_analyse_rich_particles_mass_change)
print(output_data)
_plot_rich_particles_mass_change(output_data=output_data, 
                                    showfig       = False,
                                    savefig       = True)"""
