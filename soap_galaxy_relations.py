...
def _stelmass_gasmass_compare_aperture(simulation_run = ['L100_m6', 'L0025N0752'], 
    ...
    for sim_run_i, sim_type_i, snap_i in zip(simulation_run_list, simulation_type_list, snapshot_no_list):
        ...
        for aperture in aperture_list:
            ...
            line, = plt.plot(bin_centers, medians, color='k', linewidth=1, label='%s kpc'%aperture)
    ...
    if savefig:
        ...
        plt.savefig("%s/soap_galaxy_relations/%s%s_%s_aperturecheck_stelmass-%s%s_%s.%s" %(fig_dir, ('redshift/' if format_z else ''), '_'.join(run_name_list), '_'.join(str(x) for x in snapshot_no), mass_type, ('_fraction' if plot_fractions else ''), savefig_txt_save, file_format), format=file_format, bbox_inches='tight', dpi=600) 
        print("\n  SAVED: %s/soap_galaxy_relations/%s%s_%s_aperturecheck_stelmass-%s%s_%s.%s" %(fig_dir, ('redshift/' if format_z else ''), '_'.join(run_name_list), '_'.join(str(x) for x in snapshot_no), mass_type, ('_fraction' if plot_fractions else ''), savefig_txt_save, file_format))
    if showfig:
        plt.show()
    plt.close()
...
# Atomic gas 30 vs 50 pkpc
_stelmass_gasmass_compare_aperture(simulation_run = ['L100_m6'], simulation_type = ['THERMAL_AGN_m6'], snapshot_no     = [127],        
                mass_type = 'atomic_hydrogen_mass',    # [ molecular_and_atomic_hydrogen_mass / molecular_hydrogen_mass / atomic_hydrogen_mass ]
                 centrals_or_satellites = 'both',      # [ both / centrals / satellites ]
                    aperture_list = [30, 50], 
